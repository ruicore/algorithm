...
{
	char* Plaintext = NULL;
	int Ciphertext_text_num = 0, key_num = 0;
	int *key_order = NULL;
	int i = 0, j = 0, temp = 0;

	Ciphertext_text_num = strlen(Ciphertext), key_num = strlen(key);
	Plaintext = malloc(sizeof(char)*(Ciphertext_text_num + 1));//主调函数内free
	Plaintext[Ciphertext_text_num] = NULL;

	key_order = Get_order(key);
	for (i = 0; i < key_num; i++)
	{
		j = key_order[i];
		while (j<Ciphertext_text_num)
		{
			Plaintext[j] = Ciphertext[temp];
			j += key_num;
			temp++;
		}
	}
